...
function pickOrderList(categoryName) {
  ...
  if (n.indexOf('除湿') !== -1) return DEHUMIDIFIER_ORDER;
  if (n.indexOf('空调') !== -1) return AC_ORDER;
  return null;
}
...
function sortKeyForGuide(g, orderList) {
  ...
  for (let i = 0; i < orderList.length; i++) {
    const t = orderList[i];
    if (nm.indexOf(t) !== -1 || t.indexOf(nm) !== -1) return i;
  }
  return 1000 + (g.sortOrder || 0) * 0.001 + (g.id || 0) * 1e-6;
}

function sortGuidesByDisplayOrder(guides, categoryName) {
  const orderList = pickOrderList(categoryName);
  if (!orderList) {
    return guides.slice().sort((a, b) => (a.sortOrder || 0) - (b.sortOrder || 0) || (a.id || 0) - (b.id || 0));
  }
...
function sortCategoriesForSidebar(categories) {
  ...
  const rank = (name) => {
    const n = (name || '').trim();
    if (n === '空调') return 0;
    if (n === '除湿机' || n.indexOf('除湿') !== -1) return 1;
    return 2;
  };
  list.sort((a, b) => {
