---
name: 更新发布
description: 通过 SSH 在腾讯云服务器上拉取代码并仅重建后端与前端（不整栈重启、不重启 MySQL、不修改 DB）。执行前需先阅读 connectToTxCloud.md 获取 SSH 与服务器信息。
---

# 更新发布（Vino_test）

当用户要求「更新发布」「发布」「部署」时，按以下流程执行。**部署方式**：先 SSH 到服务器，再 git pull 拉取最新代码，再编译更新发布（仅 vino-backend、vino-frontend）。
...
- **首页配置**：管理 **自助预约(大/小)**、**我的商品**（及 Vino 产品、甄选推荐、探索 VINO 等在后台对应入口）。红框处可设置 **板块整体偏移(px)**，仅移动首页配置管理对应区块，不移动首页动画配置（开场动画、首页 Logo、首页背景）。**自助服务 / 服务产品** 已从首页与后台配置表移除；历史 `home_configs` 数据可用仓库内 `backend/src/scripts/deleteRemovedHomeSections.js` 在容器内一次性清理。
...
1. **SSH 到服务器：在项目目录下 git pull**
2. **服务器：仅重建并启动 vino-backend、vino-frontend**（不 `docker compose down`，不重启 MySQL）

（若需先从本机推代码，再执行下方「可选：本地 Git 上传」。）
...
## 第二步：仅构建并更新应用（不 down、不动 DB）

在同一台服务器上，进入项目目录，**仅**对 vino-backend、vino-frontend 重新构建并启动；不执行 `docker compose down`，不重启 vino-mysql：
...
- 仅会重建并重启 **vino-backend**、**vino-frontend**，MySQL 保持运行。
- 构建时间可能较长，命令会等待执行结束。
...
# 2. 仅构建并更新后端与前端（不 down、不动 volume）
ssh -o HostKeyAlgorithms=+ssh-rsa -o PubkeyAcceptedKeyTypes=+ssh-rsa -o StrictHostKeyChecking=no -i F:/ItsyourTurnMy/backend/deploy/test.pem ubuntu@106.54.50.88 "sudo bash -c 'cd /home/ubuntu/Vino_test && docker compose up -d --build vino-backend vino-frontend'"
```
