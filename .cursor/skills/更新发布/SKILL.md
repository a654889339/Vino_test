---
name: 更新发布
description: 将本地代码上传到 Git，并通过 SSH 在腾讯云服务器上拉取代码，仅重启后端与前端进程（不整栈重启 Docker）。执行前需先阅读 connectToTxCloud.md 获取 SSH 与服务器信息。
---
...
1. **本地 Git：提交并推送**
2. **SSH 到服务器：拉取代码**
3. **服务器：仅重启应用进程**（重新构建并启动后端、前端容器，不执行 `docker compose down`，不重启 MySQL）
...
## 第三步：仅重启应用进程（不整栈重启 Docker）

在同一台服务器上，进入项目目录，**仅**对后端与前端重新构建并启动，不执行 `docker compose down`，不重启 MySQL（connect 文档中注明使用 `docker compose`）：

```bash
ssh -o HostKeyAlgorithms=+ssh-rsa -o PubkeyAcceptedKeyTypes=+ssh-rsa -o StrictHostKeyChecking=no -i <密钥路径> ubuntu@106.54.50.88 "sudo bash -c 'cd /root/Vino_test && docker compose up -d --build vino-backend vino-frontend'"
```

- 仅会重建并重启 **vino-backend**、**vino-frontend** 两个服务，MySQL 保持运行，避免整栈停机。
...
6. **整栈重启**：若确需重启全部服务（含 MySQL），再使用 `docker compose down && docker compose up -d --build`；日常更新部署仅用第三步即可。
