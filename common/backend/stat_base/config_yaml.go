...
// FileConfig 与 common/config/stat/*.yaml 结构一致。
...
// LoadFileConfig 读取 YAML；path 为空返回零值结构。
func LoadFileConfig(path string) (*FileConfig, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("read stat config: %w", err)
	}
	var c FileConfig
...
// Resolve 合并 YAML 与默认本地目录（通常 cfg.Log.StatDir）。
